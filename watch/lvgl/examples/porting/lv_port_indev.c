...
static bool touchpad_is_pressed(void)
{
    ...
    if (TouchDev.x[0]==-1 && TouchDev.y[0]==-1)
    {
        return false;
...
static void touchpad_get_xy(int32_t * x, int32_t * y)
{
    ...
    (*x) = (int32_t)TouchDev.x[0];
    (*y) = (int32_t)TouchDev.y[0];
    //St7789_FillColor(TouchDev.x[0], TouchDev.y[0], TouchDev.x[0]+10, TouchDev.y[0]+10, 0xF800); 
}
