 ...
 */
osStatus_t I2C_Manager_Write_DMA(I2C_HandleTypeDef *hi2c, uint16_t DevAddress,
  ...
  hal_status = HAL_I2C_Mem_Write_DMA(hi2c, DevAddress, MemAddress, MemAddSize,
                                     pData, Size);

  if (hal_status != HAL_OK) {
    I2C_Check_And_Attempt_Recovery();
    // 启动失败 (例如 I2C_BUSY)
 ...
 */
osStatus_t I2C_Manager_Read_DMA(I2C_HandleTypeDef *hi2c, uint16_t DevAddress,
                                uint16_t MemAddress, uint16_t MemAddSize,
                                uint8_t *pData, uint16_t Size,
                                uint32_t timeout) {
  HAL_StatusTypeDef hal_status;
  ...
  // 2. 启动 I2C DMA 读传输
  hal_status = HAL_I2C_Mem_Read_DMA(hi2c, DevAddress, MemAddress, MemAddSize,
...
void HAL_I2C_ErrorCallback(I2C_HandleTypeDef *hi2c) {
  if (hi2c->Instance == hi2c1.Instance) {
    ...
    const char *msg = ">>> I2C ErrorCallback ENTER <<<\r\n";
    Uart1_Tx.Uart_Send(&Uart1_Tx, (uint8_t*)msg, strlen(msg));
    // **至关重要**：即使出错了，也必须释放信号量！
    // 否则，等待的任务将永久卡死 (死锁)
    osSemaphoreRelease(i2cDmaCpltSemHandle);
  }
